...
def rich_house():
	...
	middle_roof = tetto_terrazzo(PI/2.5,fmi,0.1,0)

	first_floor = make_floor(1)
	first_floor = STRUCT([first_floor,T([1,2,3])([-0.6,-0.8,3.2])(middle_roof)])

	middle_floor = make_floor(2)
	middle_floor = STRUCT([middle_floor,T([1,2,3])([-0.6,-0.8,3.2])(middle_roof)])

	middle_floors = STRUCT([middle_floor,T(3)(3.1)(middle_floor)])

	casa = STRUCT([first_floor,T(3)(3.1)(middle_floors)])

	last_roof = tetto_terrazzo(PI/2.5,fmi,1,1)
	
	last_floor = make_floor(2)
	last_floor = STRUCT([last_floor,T([1,2,3])([-0.5,-0.8,3.2])(last_roof)])

	casa = STRUCT([casa,T(3)(3.1)(middle_floor)])

	casa = STRUCT([casa,T(3)(9.3)(last_floor)])
...
def make_floor(floor):
	...
	floor = makeFloor("10_svg/1_muri_est.lines")
